...
from core.function import validate_sel_baseline
...
def main():
    ...
    for extra_sample in range(1, config.TRAIN_SEL.MAX_CHOOSING_NUM - config.TRAIN_SEL.PRECHOOSING_NUM + 1):
        ...
        perf_indicator = validate_sel_baseline(config, valid_loader, model, criterion, extra_sample, 0,
                                      output_dict, valid_dataset)

        # output the result
        logger.info("=> saving classification result into {}".format(
            os.path.join(config.OUTPUT_DIR, 'result_sel_baseline_extra_{extra_sample}_{acc_avg}.json'.format(
                                            extra_sample=extra_sample, acc_avg=perf_indicator))
        ))
        output_file = open(os.path.join(config.OUTPUT_DIR,
                                        'result_sel_baseline_extra_{extra_sample}_{acc_avg}.json'.format(
                                            extra_sample=extra_sample, acc_avg=perf_indicator)),
                           'w')
